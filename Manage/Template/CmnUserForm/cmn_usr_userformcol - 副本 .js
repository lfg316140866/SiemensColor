...
var CreateCtlCfgPanel = {
    ...
    Init: function (PanelCfgJson,DataJson) {
        ...
        $(".jscAddOptionListItem").off("click").on("click", function () {
            ...
            var _item = $(this).parent("div").find(".jscOptionListItem");

            //如果是类型
            if ($(this).parents(".ControlCfgCol").attr("type") == CreateCtlCfgPanel.Type.Sorts) {
                _item.append('<div class="jscOptionListItemInput" style="border:1px #000 solid">' +
                           '分类名称:<input type="text" t="Name" style="width:100px;border: 1px solid #ccc;"/>' +
                           '<br>Sql:<textarea t="FillSql" style="width:250px;height:100px; font-size:12px;border: 1px solid #ccc;"></textarea>  ' +
                           '<br>字段名称:<input t="FieldName" type="text" style="width:100px;border: 1px solid #ccc;" />' +
                           '<button style="border: 1px solid #ccc;display:inline;zoom:1;" class="jscDelOptionListItem">删除</button>' +
                      ' </div>');
            }
            else {
                _item.append('<div class="jscOptionListItemInput">' +
                                'key:<input type="text" class="key"style="width:60px;border: 1px solid #ccc;" />   ' +
                                'value:<input type="text" style="width:100px;border: 1px solid #ccc;"  class="value" />' +
                                '<button style="border: 1px solid #ccc;display:inline;zoom:1;" class="jscDelOptionListItem">删除</button>' +
                           ' </div>');
            }
    ...
    else if (cfg.type == CreateCtlCfgPanel.Type.Sorts) {
        _html = '<div class="jscOptionListItem" ></div><button class="jscAddOptionListItem"  style="border: 1px solid #ccc;">添加</button><button class="jscDelOptionListItemAll"  style="border: 1px solid #ccc;">删除所有</button></br>';
    }
    ...
    GetControlValue: function ($control) {
        ...
        else if (_type == CreateCtlCfgPanel.Type.Sorts) {
            _val = "["
            $control.find(".jscOptionListItemInput").each(function (index, item) {
                var _delimiter = ",";
                if (index == $control.find(".jscOptionListItemInput").size() - 1) { _delimiter = ""; }
                _val += "{" + '"' + "Name" + '":"' + $(item).find("input[t=Name]").val() + '"' + ","
                    + '"' + "FieldName" + '":"' + $(item).find("input[t=FieldName]").val() + '"' + ","
                    + '"' + "FillSql" + '":"' + $(item).find("textarea[t=FillSql]").val() + "\"}" + _delimiter;
            })
            _val += "]";
        }
    ...
    SetControlValue: function (cfg) {
        ...
        $(".ControlCfgCol").each(function (index, item) {
            if ($(item).attr("type") == CreateCtlCfgPanel.Type.Text) {
                ...
                //生成类型选择配置界面
            else if ($(item).attr("type") == CreateCtlCfgPanel.Type.Sorts) {
                if (typeof cfg[$(item).attr("key")] == "object") {
                    var _item  =   $(item).parent("div").find(".jscOptionListItem");
                    $.each(cfg[$(item).attr("key")], function (index, val) {
                        _item.append('<div class="jscOptionListItemInput" style="border:1px #000 solid">' +
                           '分类名称:<input type="text" t="Name" value="'+val.Name+'" style="width:100px;border: 1px solid #ccc;"/>' +
                           '<br>Sql:<textarea t="FillSql" value="' + val.FillSql + '" style="width:250px;height:100px; font-size:12px;border: 1px solid #ccc;">"' + val.FillSql + '"</textarea>  ' +
                           '<br>字段名称:<input t="FieldName" value="' + val.FieldName + '" type="text" style="width:100px;border: 1px solid #ccc;" />' +
                           '<button style="border: 1px solid #ccc;display:inline;zoom:1;" class="jscDelOptionListItem">删除</button>' +
                      ' </div>');
                    });
                }
