...
// 		}
   ...
   /**
	...
	public static function AddLimitToSelect($sql,$limitSentence){
		//判断是否是select 语句，不是select 语句是不能加limit
...
// 		else { $_sql = str_replace($_result," count(1) ",$_sql); }
		...
		if ($_bracketLst != null) { $_sql = SqlAnalyse::recoverBracket($_sql, $_bracketLst); }
		if ($_selectLst != null) { $_sql = SqlAnalyse::recoverSubSelect($_sql, $_selectLst); }
		
		return " select count(1) from (" . $_sql . ") cmn_main_table ";
	}
	...
	//------------------------------------------------------
	/// <summary>
	/// 获取Select语句中，select字段的列表（包含字段名前缀，中括号等）
	/// </summary>
	/// <param name="sql">sql语句</param>
	/// <returns>select字段的列表（如果有化名，取得的是化名）</returns>
	public static function GetSelectFieldList($sql) {
		//去掉括号 add by sulgar at 2013/6/7
		$_selectLst = array();
		$sql =SqlAnalyse::WipeBracket($sql, $_selectLst);
		
		$_fields = explode(',',SqlAnalyse::getSelectFields($sql));
		$_tmpStr = "";
		$_fieldList = array();
	
		for ($_i = 0; $_i < count($_fields); $_i++) {
			$_tmpStr =explode(' ',trim($_fields[$_i]));

			if (count($_tmpStr)>=2) { //如果有空格的话，说明可能有化名，需要取得化名
				$_tmpStr =trim($_tmpStr[1]);
			}
			else{$_tmpStr =trim($_tmpStr[0]);}
	
			$_fieldList[$_i] = $_tmpStr;
		}
	...
	//------------------------------------------------------
	/// <summary>
	/// 获取Select语句中，select字段的列表（去除了字段名前缀，中括号等，仅仅是干净的字段名）
	/// </summary>
	/// <param name="sql">sql语句</param>
	/// <returns>字段名列表（干净）</returns>
	public static function GetSelectFieldCleanList($sql) {
		$_fieldList = SqlAnalyse::GetSelectFieldList($sql);
	
		$_loc = 0;
		for ($_k = 0; $_k <count($_fieldList); $_k++) {
			$_loc =strpos($_fieldList[$_k],".");
			if ($_loc!=false && $_loc >= 0) {
				$_fieldList[$_k] =substr($_fieldList[$_k],$_loc + 1);
			}
	
			$_fieldList[$_k] =str_replace("[","",$_fieldList[$_k]);
			$_fieldList[$_k] =trim(str_replace("]","",$_fieldList[$_k]));
		}
