...
class FileOpt{
	...
	public static function Upload(){
		...
		//其次 要么是藏在请求头里面的文件数据 或者是base64
			...
			if(isset($_fileData)){
				...
				preg_match('/attachment;\s+name="(.+?)";\s+filename="(.+?)"/i',$_SERVER['HTTP_CONTENT_DISPOSITION'],$info)){
					...
					if($_realFileName==""){
						$_realFileName=date("YmdHis").mt_rand(10000,99999).'.png';
					}
				}
				...
				if(strpos($_fileData,"base64")!=false){
					...
					if($_realFileName==""){
						$_realFileName=date("YmdHis").mt_rand(10000,99999).'.'
		...
		else{$_fileName = date("YmdHis").mt_rand(10000,99999).'.png';}
		...
		if(filesize($_saveRootPath."/".$_tempFileName) > $_limitSize){
			...
			$_receiptData["err"]="";
			$_receiptData["msg"]["localname"]=$_fileName;
			$_receiptData["msg"]["url"]=FileOpt::GetRequestUri().$_requestRootPath."/".$_fileName;
	...
	//根据路径删除服务器端的文件
 	...
 	//	$_filePath = dirname(dirname(dirname(dirname(__FILE__)))).$_filePath;
		...
		if(@unlink ($_filePath) == false){
			return FileOpt::jsonEncode(array("IsSuccess"=> "1","ErrMsg"=> "删除成功！"));
		}
		else{
			return FileOpt::jsonEncode(array("IsSuccess"=> "0","ErrMsg"=> "删除失败！"));
		}
 	}
	...
	private static function SaveFile($fileData,$fileSaveName,$isInputFile){
		 ...
		 if($isInputFile){
		 	...
		 	//保存文件count($fileData) === 
		 	return file_put_contents($fileSaveName, $fileData);
		 }
	}
