	...
	public static function GetUserFormInfo(&$retArray)
	{
		...
		$_dt = DB::getArray("
			select userformid,formdesc,formurl,pagesize,b.userformmoduledesc,
				...
				d.file_name as EditTemplateFileName,e.file_name as JsTemplateFileName,
				form_exd_cfg
			from cmn_usr_userform as a
				...
				left join cmn_template as e on a.js_template_id = e.template_id
		...
		$retArray["JsTemplateFileName"]= $_dt[0]["JsTemplateFileName"];	
	
		if ($_dt[0]["form_exd_cfg"] != "") {
			$retArray["ExdCfg"] = $_dt[0]["form_exd_cfg"];
		}
...
// 	}
	...
	private static function GetSonMenu(&$moduleObj,$userFormDT) {
		...
		foreach ($userFormDT as $_item) {
			if ($_moduleID == $_item["userformmoduleid"]) { //有用户表单
				...
				if ($_item["form_exd_cfg"] != "") {
					$_joUserForm["ExdCfg"] = $_item["form_exd_cfg"];
				}
	...
	public static function GetMenu(&$retArray) {	
		...
		$_data = DB::getArray($_sql);
		if ($_data == null &&  count($_data) <= 0) { return $retArray->Error("无菜单数据！")->ToString(); }
		...
		if (array_key_exists("UserForms",$_moduleObj)) {
			$_dt = DB::getArray("
				select userformmoduleid,userformmoduledesc,i.icon_path
				from cmn_usr_userformmodule  ufm
					left join cmn_icon i on ufm.icon_id=i.icon_id
				where userformmoduleid ='" . $_userFormModuleID . "'");
			
			$_array = array();
			$_tmpObj = array();
			
			$_tmpObj["ModuleID"] = $_userFormModuleID;
			$_tmpObj["ModuleDesc"] = $_dt[0]["userformmoduledesc"];
			$_tmpObj["ModuleIcon"] = $_dt[0]["icon_path"];
			$_tmpObj["UserForms"] = $_moduleObj["UserForms"];
			
			if (array_key_exists("Sons", $_moduleObj)) {
				$_tmpObj["Sons"] = $_moduleObj["Sons"];
			}
			
			$_array[]= $_tmpObj;
			
			$retArray["data"] = $_array;
		}
		else if (array_key_exists("Sons", $_moduleObj)) { $retArray["data"] = $_moduleObj["Sons"]; }
	...
	public static function ExecSql(&$retArray){
		...
		//检测权限
		if (!ItfFunc::IsHasRight($retArray)) { return $retArray->ToString(); }
		//获取生成加密sql语句
		$_sql = Request::Get("ExecSql"); 
		 //获取参数
		$_param = trim(Request::Get("param"));
		//条件
		$_where =Request::Get("where"); 
		...
		if($_param == ""){ 
		 	if ($_where != "")
            {
                $_where = ItfFunc::FormatFieldName($_sql, $_where);
                $_sql = SqlAnalyse::AddWhere($_sql, $_where);
            }
		}
		else{
			//有查询参数的话意味着是autocomplete
			$_dispFieldCount = trim(Request::Get("DisplayFieldCount"));
			$_count = trim(Request::Get("Limit"));
	    	...
	    	if($_where!=""){
	    	 ...
	    	 //将sql语句格式化成autocomplete控件需要的sql语句
           $_sql = ItfFunc::FormatAutoCompleteSql($_sql, $_dispFieldCount, $_count, $_where);
	...
	public static  function GetCSV(&$retArray){
		...
		for($_k = 1; $_k<=$_pageCount;$_k++){
			...
			if($_k==1){
				...
				if(empty($_arrList)) {
					return UserFormProcessItf::i("没有符合条件的数据");
				}
		...
		$_data = iconv('utf-8','utf-16le',$_data);
		
		return chr(255).chr(254).$_data;
	}
	//----------------------------------------------------------
	/// <summary>
	/// 格式化Csv文件中的某一个字段内容
