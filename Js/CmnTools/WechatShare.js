...
function CallJsApiWXConfigItf(itfUrl) {
    ...
    var _rootUrl = Cmn.Func.GetAbsoluteUrl("/");
    

    if (_WxVersion >= "6.0.2") {
        var _showDebugLog = Cmn.Func.GetParamFromUrl("ShowWeiXinDebugLog", location.href);
        if (_showDebugLog == "1") {
            _showDebugLog = "&Debug=1";
        }

        if (itfUrl.indexOf(_rootUrl) >= 0) {
            // 微信Js API Config
            CmnAjax.GetFile(itfUrl + (itfUrl.indexOf("?") >0 ? "&" : "?")+"url=" + encodeURIComponent(location.href) + _showDebugLog, function (data) {
                o = document.createElement("script");
                o.text = data;
                document.documentElement.childNodes[0].appendChild(o);
            });
        } else {
            CmnAjax.PostData(itfUrl + (itfUrl.indexOf("?") >0 ? "&" : "?")+ "url=" + encodeURIComponent(location.href) + _showDebugLog, function (data) {
                //alert(data);
            });
        }
    }
